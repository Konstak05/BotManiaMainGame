...
public class EnemybotAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i2 = 0; i2 < cosmetics.Length; i2++){cosmetics[i2].SetActive(i2 == randomCosmeticIndex);}
    ...
    private void Update()
    {
        ...
        if(playerInSightRange && HP > 0 && IsDead == 0 && EnemyStartDelay == 1){
            ...
            if(IsPatrolling == 0 && EnemyStartDelay == 1){
                CancelInvoke("StartEnemy"); CancelInvoke("Attack1A"); CancelInvoke("Attack1B"); CancelInvoke("Attack1C"); CancelInvoke("Attack2A"); CancelInvoke("ChasePlayer");
                HealingParticle.Stop();
                Gun.SetActive(false); Arm1.SetActive(false); Arm2.SetActive(false); AlertSign.SetActive(false); HealthBar.SetActive(false);
                LIFE = 0;
                IsPatrolling = 1;
                IsSearching = 1;
                sightRange = sightRangeWhenLost;

                if(hascounted == 2 && GlobalData.GetEnemyCount() > 0){GlobalData.SetEnemyCount(GlobalData.GetEnemyCount() - 1);}
            }
            animator.SetFloat("Speed", navMeshAgent.velocity.magnitude);
            int InPatrolMode = UnityEngine.Random.Range(1, PatrolChance);
            if(InPatrolMode == 1){Invoke("PatrolGo", 1f);}
        }

        //Deathcode
        if(HP <= 0){
            gameObject.GetComponent<Collider>().enabled = false;
            Arm1.SetActive(false); Arm2.SetActive(false); Gun.SetActive(false);
            if(Gun2 != null){Gun2.SetActive(false);}
            animator.SetFloat("Speed", 0);
            CancelInvoke("StartEnemy"); CancelInvoke("Attack1A"); CancelInvoke("Attack1B"); CancelInvoke("Attack1C"); CancelInvoke("Attack2A"); CancelInvoke("ChasePlayer"); CancelInvoke("MeleeAttack"); CancelInvoke("MeleeAttack2");
            HealingParticle.Stop();
            ...
            for (int i = 0; i < cosmetics.Length; i++) {cosmetics[i].SetActive(false);}
        }
    }
    ...
    void StartEnemy()
    {
        ...
        Gun.SetActive(false);
        if(Gun2 != null){Gun2.SetActive(false);}
        CancelInvoke("Attack2A"); CancelInvoke("ChasePlayer"); CancelInvoke("MeleeAttack"); CancelInvoke("MeleeAttack2");
    ...
    void Attack1C()
    {
        if(HP < HPMAX && HP > 0){HP = HP + HPMAX*Mag/4f;}
        IsHealing = 0;
        Arm1.SetActive(false);
        Arm2.SetActive(false);
        Invoke("StartEnemy", StartEnemyAndRefresh);
    ...
    void MeleeAttack()
    {
        ...
        if(playerInMeleeRange){Invoke("MeleeAttack2", 0.1f);}
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        {
            if(IsHealing == 1){
                CancelInvoke("Attack1A"); CancelInvoke("Attack1B"); CancelInvoke("Attack1C");
                Arm1.SetActive(false); Arm2.SetActive(false);
                HealingParticle.Stop();
                Invoke("StartEnemy", StartEnemyAndRefresh);
