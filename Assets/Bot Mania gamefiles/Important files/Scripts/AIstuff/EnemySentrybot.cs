...
public class EnemySentrybot : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(playerInSightRange && HP > 0 && IsDead == 0 && EnemyStartDelay == 1){
            if(IsSearching == 1){
                ...
                hascounted = 2;
                if(IsDead == 0 && GlobalData.GetEnemyCount() >= 0 && LIFE == 1){GlobalData.SetEnemyCount(GlobalData.GetEnemyCount() + 1);}
            }
        }
        else{
            if(IsPatrolling == 0 && EnemyStartDelay == 1){
                CancelInvoke("StartEnemy"); CancelInvoke("Attack2A"); CancelInvoke("ChasePlayer");
                AlertSign.SetActive(false); HealthBar.SetActive(false);
                LIFE = 0;
                IsPatrolling = 1;
                IsSearching = 1;
    ...
    void StartEnemy()
    {
        ...
        AlertSign.SetActive(false);
        CancelInvoke("Attack2A"); CancelInvoke("ChasePlayer");
