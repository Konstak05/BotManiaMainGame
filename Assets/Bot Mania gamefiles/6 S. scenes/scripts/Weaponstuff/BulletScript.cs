...
public class BulletScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (Physics.Raycast(initialPosition, transform.forward, out hit , Mathf.Infinity, layerMask))
        {
            Debug.DrawLine(initialPosition, hit.point, Color.red, 5f);
        }
    }
...
void Update()
{
    if(NormalBullet){
        ...
        if (hit.collider != null && distanceTraveled >= hit.distance - destroyDistance)
        {
            ...
            if(i2 == RandomBulletholeSprite){
            Instantiate(BulletHole[i2], hit.point + hit.normal * 0.05f, rotation);
            }
