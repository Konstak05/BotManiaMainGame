...

{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, MaxRange, LayerMask.GetMask("Objects")) && ToggleUI.PauseMenu == 0 && Gunscript.GunEquipped == 3)
        {
            ...
            if (Input.GetMouseButtonDown(0) && !HasSticked)
            {   
            ...
            if (Input.GetMouseButtonUp(0) && HasSticked && !HastoStick){HastoStick = true;}

            if (Input.GetMouseButtonDown(0) && HastoStick && !HasSticked2)
            {   
        ...
        if (ToggleUI.PauseMenu == 1 | Gunscript.GunEquipped != 3 && AlreadyChanged == true){
            ...
            if(Input.GetKeyDown(KeyCode.R) && ToggleUI.PauseMenu == 0 && Gunscript.GunEquipped == 3){
              if(Cantclick == true){
              float audioVolume = PlayerPrefs.GetFloat("AudioVolume");
              float masterVolume = PlayerPrefs.GetFloat("MasterVolume");
              Sound.volume = audioVolume * masterVolume;
              ...
              HasSticked2 = false;
              HastoStick = false;
              if(hit.collider.GetComponent<Joint>() != null){
              targetJoint = hit.collider.GetComponent<Joint>();
              if(targetJoint != null)
              {
              Destroy(targetJoint);
              }
